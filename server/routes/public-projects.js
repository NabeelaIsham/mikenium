...
router.get('/',async(req,res)=>{
  ...
  res.set('Cache-Control','no-store').json({projects:rows.map(row=>({id:row.id,name:row.name,slug:row.slug,description:row.description,category:row.category,result:row.public_result,tags:row.tags||[],imageUrl:publicUrlOr(row.image_url),projectUrl:publicUrlOr(row.project_url),theme:row.theme,featured:row.is_featured,client:row.client||'',createdAt:row.created_at}))});
});
