...
import { getPublicServices } from '../../services/public-api';

const navItems=[['Home','/'],['Services','/services'],['Portfolio','/portfolio'],['Products','/products'],['Pricing','/pricing'],['Blog','/blog'],['About','/about']];
const serviceIcons={code:I.MonitorSmartphone,mobile:I.Smartphone,design:I.PenTool,cloud:I.CloudCog,commerce:I.ShoppingBag,support:I.Headphones,security:I.ShieldCheck,marketing:I.ChartNoAxesCombined,database:I.Database,automation:I.Workflow};
const serviceTones=['blue','cyan','violet','sky','indigo','teal'];
const models=[
...
function Logo(){return <a className="public-logo" href="/" aria-label="Mikenium home"><BrandLogo src={companyContact.logoUrl} alt={companyContact.logoAlt}/></a>}
    ...
    <main>
      <section className="services-hero">
        ...
        <div className="services-hero-copy">
          <span className="services-kicker"><i/> OUR SERVICES <b>{services.length?`01 — ${String(services.length).padStart(2,'0')}`:'DATABASE'}</b></span>
      ...
      <section className="capabilities reveal" id="capabilities">
        ...
        {servicesLoading?<div className="services-data-state"><I.LoaderCircle className="services-spin"/> Loading services...</div>:services.length?<div className="capability-grid">{services.map((service,i)=>{const Icon=serviceIcons[service.icon]||I.MonitorSmartphone;return <article className={'capability-card '+serviceTones[i%serviceTones.length]} key={service.id}><div className="capability-top"><span className="capability-icon"><Icon/></span><small>{String(i+1).padStart(2,'0')}</small></div><span className="capability-tag">{service.tagline||service.category.toUpperCase()}</span><h3>{service.name}</h3><p>{service.description}</p><ul>{service.features.map(item=><li key={item}><I.Check/>{item}</li>)}</ul><a href="/#contact">Discuss this service <I.ArrowUpRight/></a><i className="card-beam"/></article>})}</div>:<div className="services-data-state empty"><I.FolderOpen/><h3>No published services yet</h3><p>Services published by the Super Admin will appear here.</p></div>}
      </section>
