  ...
  return <div className="public-site about-page">
    ...
    <main>
      ...
      <section className="about-team reveal">
        <div className="team-intro"><span className="section-kicker">THE PEOPLE BEHIND THE PRODUCT</span><h2>Small team.<br/><span>Big commitment.</span></h2><p>Developers, designers, and strategists who love turning ideas into products that make a difference.</p><ArrowLink href="/#contact">Join our team</ArrowLink></div>
        <div className="team-grid">{team.map(([initials,name,role,focus],i)=><article key={name} className={'team-card team-'+i}><div className="member-portrait"><span className="portrait-ring"/><strong>{initials}</strong><small>{focus}</small></div><div className="member-info"><span><b>{name}</b><small>{role}</small></span><a href="/#contact" aria-label={`Contact ${name}`}><I.Linkedin/></a></div></article>)}</div>
      </section>

      <section className="about-cta reveal"><div className="cta-glow"/><span className="cta-icon"><I.Send/></span><div><small>HAVE SOMETHING AMBITIOUS IN MIND?</small><h2>Let’s build what’s next.</h2><p>Tell us where you want to go. We’ll help design the way forward.</p></div><ArrowLink href="/#contact">Start a conversation</ArrowLink></section>
    </main>

    <footer id="contact"><div className="footer-glow"/><div className="cta"><div><span className="cta-label"><i/> LET’S CREATE TOGETHER</span><h2>Ready to build software<br/>that moves your business?</h2><p>Bring us the challenge. We’ll bring the strategy, design, and engineering to turn it into a product people value.</p></div><div className="cta-actions"><SiteButton>Start a Conversation</SiteButton><a href="mailto:hello@mikenium.com">hello@mikenium.com <I.ArrowUpRight/></a></div></div><div className="footer-main"><div className="footer-about"><img src="/assets/mikenium-logo-transparent.png" alt="Mikenium — Building Smarter Software"/><p>We design and engineer secure digital products that help ambitious businesses grow with confidence.</p><span className="footer-socials">{[I.Facebook,I.Linkedin,I.Twitter,I.Github,I.Instagram].map((Icon,i)=><a href="/" aria-label="Social link" key={i}><Icon/></a>)}</span></div><div className="footer-links"><div><h4>Company</h4>{[['About Us','/about'],['Our Work','/#portfolio'],['Careers','/#contact'],['Contact','/#contact']].map(([label,href])=><a href={href} key={label}>{label}</a>)}</div><div><h4>Expertise</h4>{['Web Development','Mobile Apps','Cloud Solutions','UI/UX Design'].map(x=><a href="/services#capabilities" key={x}>{x}</a>)}</div><div><h4>Resources</h4>{['Insights','Products','Pricing','Client Stories'].map(x=><a href="/#blog" key={x}>{x}</a>)}</div></div><div className="footer-contact"><span><I.MapPin/></span><div><h4>Let’s talk about your next idea.</h4><a href="mailto:hello@mikenium.com">hello@mikenium.com</a><a href="tel:+94112345678">+94 11 234 5678</a><small>Colombo, Sri Lanka</small></div></div></div><div className="footer-bottom"><p>© 2026 Mikenium. All rights reserved.</p><nav><a href="/">Privacy Policy</a><a href="/">Terms of Service</a><a href="/">Cookie Policy</a></nav><span><i/> All systems operational</span></div></footer>
  </div>
}
